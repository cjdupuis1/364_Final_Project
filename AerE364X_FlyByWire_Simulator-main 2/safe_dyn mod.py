...
class dynamic_model_interactor(object):
    ...
    def interactor_update(self):
        ...
        with self.model.model_lock:
            self.model.ROTCMD = int(self.rot_slider.val*1000)
            pass
        
        with self.model.model_lock:
            self.model.FWDCMD = int(self.fwd_slider.val*1000)
            pass
    ...
    def interactor(self):
        ...
        try: 
            ...
            self.subfig_xy = pl.subplot(2,3,1)
            pl.axis((-2,2,-2,2))
            self.subfig_yz = pl.subplot(2,3,3)        
            pl.axis((-2,2,-2,2))
            self.subfig_xz = pl.subplot(2,3,2)
            pl.axis((-2,2,-2,2))
            
            self.subfig_rot_slider = pl.subplot(2,3,4)
            self.subfig_fwd_slider = pl.subplot(2,3,6)
            ...
            ax = pl.subplot(*subplotparams_from_axessubplot(self.subfig_rot_slider))
            self.rot_slider = matplotlib.widgets.Slider(ax,'ROT CMD',1.0,2.0,valinit=1.5)
            pl.xlabel('Click above to adjust\nROT CMD pulse from 0.5 to 1.5 ms')
            #self.slider.on_changed(slider_changed)
            
            ax = pl.subplot(*subplotparams_from_axessubplot(self.subfig_fwd_slider))
            self.fwd_slider = matplotlib.widgets.Slider(ax,'FWD CMD',1.0,2.0,valinit=1.5)
            pl.xlabel('Click above to adjust\nFWD CMD pulse from 0.5 to 1.5 ms')
            #self.slider.on_changed(slider_changed)
...
class dynamic_model(object):
    ...
    FWDCMD = None # rotation command, in us (1500 nominal)
            ...
            try:
                ...
                with self.model_lock:
                    ...
                    if "FWD CMD" in pulseio.PulseIns_by_pin:
                        pulseio.PulseIns_by_pin["FWD CMD"]._Add_pulse(int(self.FWDCMD),True)
